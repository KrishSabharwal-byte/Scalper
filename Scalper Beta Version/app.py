...
@app.delete("/history/{trade_id}")
@app.delete("/api/history/{trade_id}")
@app.post("/api/history/{trade_id}/delete")
async def delete_single_trade(trade_id: str, client_id: str = Depends(get_current_client_id)):
    session = get_client_session(client_id)
    db_deleted = mongo_service.delete_trade(client_id=client_id, trade_id=trade_id)
    mem_deleted = session.run_manager.delete_trade(trade_id=trade_id)
    await session.broadcast_state()
    return JSONResponse({
        "status": "success",
        "client_id": client_id,
        "trade_id": trade_id,
        "deleted": db_deleted or mem_deleted,
        "deleted_count": 1 if (db_deleted or mem_deleted) else 0,
...
async def sse_stream(request: Request, client_id: str = Depends(get_current_client_id)):
    ...
    async def event_generator():
        try:
            session.run_manager.sync_runs_from_history()
            ...
            active_preview = getattr(session, "last_astro_preview", {}) or {}
            eligible_slots = active_preview.get("eligible_slots") or []
            matrix_data = session.run_manager.get_slots_matrix(eligible_slots=eligible_slots)
...
async def get_state(client_id: str = Depends(get_current_client_id)):
    ...
    if db_trades:
        session.run_manager.master_trade_history = db_trades
        session.run_manager.sync_runs_from_history()
...
async def toggle_astro_auto_trigger(req: AstroToggleRequest, client_id: str = Depends(get_current_client_id)):
    ...
    if slot_to_toggle == "all":
        session.astro_auto_trigger["all"] = req.enabled
        for s in ["N-C", "N-P", "S-C", "S-P", "run01", "run02", "run03", "run04"]:
        ...
        canonical = SLOT_CANONICAL_MAP.get(slot_to_toggle, slot_to_toggle)
        legacy = SLOT_LEGACY_MAP.get(slot_to_toggle, slot_to_toggle)
        session.astro_auto_trigger[slot_to_toggle] = req.enabled
        session.astro_auto_trigger[canonical] = req.enabled
        session.astro_auto_trigger[legacy] = req.enabled
        if not req.enabled:
            session.astro_auto_trigger["all"] = False
        logger.info(f"[Astro Toggle API] Client '{client_id}' set Astro auto-trigger for slot {slot_to_toggle} ({canonical}) to: {req.enabled}")
    await session.broadcast_state()
    return JSONResponse({
        "status": "success",
        "client_id": client_id,
...
async def get_slots_status(client_id: str = Depends(get_current_client_id)):
    """Returns real-time status matrix for all 4 fixed slots (N-C, N-P, S-C, S-P)."""
    session = get_client_session(client_id)
    preview = getattr(session, "last_astro_preview", {}) or {}
    sig_name = preview.get("signal_name") or "NEUTRAL"
    eligible_slots = preview.get("eligible_slots") or []
    ...
    return JSONResponse({
        ...
        "basket_risk_scope": getattr(session.run_manager, "basket_risk_scope", "per_instrument"),
...
@app.get("/api/slots/events")
async def get_slots_events(limit: int = 100, client_id: str = Depends(get_current_client_id)):
    """Returns log of all slot fill and exit events."""
    session = get_client_session(client_id)
    events = list(getattr(session.run_manager, "slot_events", []))
    events.reverse()
    return JSONResponse({
        "status": "success",
        "client_id": client_id,
...
@app.get("/api/risk/basket_scope")
async def get_basket_risk_scope(client_id: str = Depends(get_current_client_id)):
    """Returns current basket stop loss scope ('per_instrument' or 'global')."""
    session = get_client_session(client_id)
    scope = getattr(session.run_manager, "basket_risk_scope", "per_instrument")
    return JSONResponse({
        "status": "success",
        "basket_risk_scope": scope,
        "scope": scope,
    })


@app.post("/api/risk/basket_scope")
async def set_basket_risk_scope(req: Dict[str, Any], client_id: str = Depends(get_current_client_id)):
    """Updates basket stop loss scope ('per_instrument' or 'global')."""
    session = get_client_session(client_id)
    scope = (req or {}).get("scope", "per_instrument")
    if scope not in ("per_instrument", "global"):
        raise HTTPException(status_code=400, detail="Scope must be 'per_instrument' or 'global'")
    session.run_manager.basket_risk_scope = scope
    return JSONResponse({
        "status": "success",
        "basket_risk_scope": scope,
        "scope": scope,
