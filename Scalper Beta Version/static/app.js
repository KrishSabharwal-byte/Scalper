...
// ── Authentication & Session Management (Phase 5) ────────────────────────
...
function setAuthToken(token, clientId) {
  try {
    ...
    if (clientId) localStorage.setItem(AUTH_CLIENT_ID_KEY, clientId);
  } catch (e) { }
  updateUserBadge();
}
...
function getStoredClientId() {
  try {
    return localStorage.getItem(AUTH_CLIENT_ID_KEY) || 'trader';
  } catch (e) {
    return 'trader';
  }
}
...
function updateUserBadge() {
  ...
  if (token && badge && userLabel) {
    badge.style.display = 'inline-flex';
    userLabel.innerText = getStoredClientId();
...
// ── Slot Tab Switcher ──────────────────────────────────────────────────────
function renderSlotTabs(runs, activeRunId) {
  ...
  sortedRuns.forEach((run, i) => {
    ...
    const isRunning = run.is_active;
    const inst = (run.instrument_name || run.config?.instrument_name || 'NIFTY').toUpperCase();
    const strike = run.locked_strike || run.strike || run.config?.strike || '--';
    const optType = run.option_type || run.config?.option_type || 'CE';
    const label = `${inst} ${strike} ${optType}`;
    const slotNum = i + 1;
    html += `
      <button class="slot-tab ${isActive ? 'active' : ''} ${isRunning ? 'running' : ''}" onclick="switchSlot('${run.run_id}')">
  ...
  for (let i = sortedRuns.length; i < 4; i++) {
    const slotId = `run0${i + 1}`;
    html += `
      <button class="slot-tab empty ${slotId === activeRunId ? 'active' : ''}" onclick="switchSlot('${slotId}')">
...
async function switchSlot(runId) {
  activeSlotId = runId;
  const runs = currentState && Array.isArray(currentState.runs) ? currentState.runs : [];
  const targetRun = runs.find(r => r.run_id === runId);
  if (targetRun) {
    currentInstrument = (targetRun.instrument_name || targetRun.config?.instrument_name || 'NIFTY').toUpperCase();
  ...
  // ── Restore per-slot Astro Auto-Trigger state ──────────────────────────────
  const slotNum = runId.replace('run0', 'Slot ');
  const slotArmed = getPersistedAstroArmed(runId);
  const isArmed = slotArmed !== null ? slotArmed : false;
  ...
  if (slotTagEl) {
    safeSetText(slotTagEl, isArmed
      ? `Astro Auto-Execution: ARMED for ${slotNum}`
      : `Auto-Trigger for ${slotNum}: OFF`);
  }
  // ─────────────────────────────────────────────────────────────────────────
...
function renderState(state) {
  ...
  renderSlotTabs(runs, currentSlotId);
...
function renderClosedSlices(tradeHistory) {
  ...
  if (displayTrades.length === 0) {
    ...
    closedSlicesTableBody.innerHTML = `
      <tr class="empty-row">
        <td colspan="12">No closed ${modeDesc}${currentInstrument} trades yet.</td>
      </tr>
    `;
    return;
  }
...
async function toggleAstroAutoExecution(enabled) {
  ...
  const slotId = activeSlotId || 'run01';
  setPersistedAstroArmed(isBool, slotId);
  ...
  const slotTagEl = document.getElementById('astroSlotTag');
  if (slotTagEl) {
    const slotNum = slotId.replace('run0', 'Slot ');
  ...
  if (currentState) {
    ...
    currentState.astro_state.auto_trigger_by_slot[slotId] = isBool;
  }
  ...
  try {
    const res = await apiFetch('/api/astro/toggle', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ slot_id: slotId, enabled: isBool }),
    });
    if (res.ok) {
      const data = await res.json();
...
function renderAstroSection(astroData) {
  ...
  // Auto Toggle Switch for Astro auto-execution — per-slot independent state
  const toggleEl = document.getElementById('astroAutoToggle');
  const slotTagEl = document.getElementById('astroSlotTag');
  const slotId = activeSlotId || 'run01';
  const slotNum = slotId.replace('run0', 'Slot ');
  ...
  let isArmed;
  const localSaved = getPersistedAstroArmed(slotId);
  if (localSaved !== null) {
    isArmed = localSaved;
  } else {
    // Fall back to server state for this specific slot
    if (autoMap[slotId] !== undefined) {
      isArmed = Boolean(autoMap[slotId]);
    } else {
      isArmed = false;
    }
    setPersistedAstroArmed(isArmed, slotId);
  }
  _userAstroArmed = isArmed;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Instantly apply this slot's saved toggle preference to DOM before any network tick
  const defaultSlot = activeSlotId || 'run01';
  const savedArmed = getPersistedAstroArmed(defaultSlot);
  const slotNum = defaultSlot.replace('run0', 'Slot ');
  if (savedArmed !== null) {
    const toggleEl = document.getElementById('astroAutoToggle');
