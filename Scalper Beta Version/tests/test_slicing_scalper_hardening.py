...
def test_atomic_strike_relock_contract_token_synchronization():
    """
    REGRESSION TEST:
    Verifies that when an active, flat slot auto-relocks across an ATM strike boundary,
    contract_token and locked_contract_token are updated atomically with contract_symbol
    and locked_contract_symbol at the exact instant handle_spot_update returns the new strike,
    with zero transient mismatch window.
    """
    contract_db = {
        (24200, "CE", "NIFTY"): {"symbol": "NIFTY 24200 CE", "token": "51000", "exchange": "NFO"},
        (24250, "CE", "NIFTY"): {"symbol": "NIFTY 24250 CE", "token": "52000", "exchange": "NFO"},
    }

    def mock_resolver(strike, option_type, instrument):
        return contract_db.get((strike, option_type, instrument))
    ...
    cfg = ScalperRunConfig(
        ...
        loss_point=10.0,
        auto_restrike=True,
        flicker_guard_ticks=3,
    )

    run = ScalperRun(cfg)
    run.contract_resolver = mock_resolver
    run.start()

    assert run.locked_strike == 24200
    assert run.locked_contract_symbol == "NIFTY 24200 CE"
    assert run.locked_contract_token == "51000"
    assert run.config.contract_token == "51000"

    # Tick 1: spot crosses to 24260 (ATM 24250) - Guard count 1 < 3
    s1 = run.handle_spot_update(24260.0, contract_resolver=mock_resolver)
    assert s1 == 24200
    assert run.locked_strike == 24200
    assert run.locked_contract_token == "51000"
    assert run.config.contract_token == "51000"

    # Tick 2: spot at 24262 (ATM 24250) - Guard count 2 < 3
    s2 = run.handle_spot_update(24262.0, contract_resolver=mock_resolver)
    assert s2 == 24200
    assert run.locked_strike == 24200
    assert run.locked_contract_token == "51000"
    assert run.config.contract_token == "51000"

    # Tick 3: spot at 24265 (ATM 24250) - Guard count 3 >= 3 -> RELOCKS!
    s3 = run.handle_spot_update(24265.0, contract_resolver=mock_resolver)
    assert s3 == 24250
    # Assert ATOMIC synchronization at the instant handle_spot_update returns
    assert run.locked_strike == 24250
    assert run.config.strike == 24250
    assert run.locked_contract_symbol == "NIFTY 24250 CE"
    assert run.config.contract_symbol == "NIFTY 24250 CE"
    assert run.locked_contract_token == "52000"
    assert run.config.contract_token == "52000"

    # Dispatched orders immediately use the synchronized new token and symbol
    dispatched = []
    run.order_callback = lambda **kwargs: dispatched.append(kwargs) or {"status": "success", "order_id": "ORD-NEW"}
    run.process_tick(140.0)
    assert len(dispatched) > 0
    assert dispatched[-1]["token"] == "52000"
    assert dispatched[-1]["symbol"] == "NIFTY 24250 CE"


def test_regression_recenter_range_floor_non_negative():
    """
    Verifies that recenter_range() enforces a minimum premium floor (MIN_PREMIUM)
    so new_range_low is never negative or below MIN_PREMIUM, even with a small
    current_price and large span.
    """
    # Small current price (e.g. 10.0) with large span (e.g. 40.0, step 8.0)
    high, low = recenter_range(current_price=10.0, original_span=40.0, slice_interval=8.0)
    assert low >= MIN_PREMIUM
    assert high > low
    assert high % 8.0 == 0 or abs((high / 8.0) - round(high / 8.0)) < 1e-4

    # Extremely low current price (e.g. 0.10) with span 100.0, step 10.0
    high2, low2 = recenter_range(current_price=0.10, original_span=100.0, slice_interval=10.0)
    assert low2 >= MIN_PREMIUM
    assert high2 > low2


def test_regression_generate_grid_and_stop_loss_clamped():
    """
    Verifies that _generate_grid() and get_range_stop_loss() never produce
    negative rungs or negative stop-losses under any low/loss_point combination.
    Reproduces production scenario: range_high=144.85, step=30, loss_point=30.
    """
    # 1. ScalperRun initialized with loss_point > range_low (mathematically negative stop loss)
    cfg = ScalperRunConfig(
        run_id="floor_test_run",
        instrument_name="NIFTY",
        option_type="PE",
        strike=24250,
        range_high=150.0,
        ...
        slice_interval=30.0,
        profit_point=10.0,
        loss_point=50.0,  # 30.0 - 50.0 = -20.0 (would be negative without clamp)
        qty_per_slice_lots=1,
    )
    run = ScalperRun(cfg)

    # Stop loss must be clamped to >= 0.0
    assert run.range_stop_loss == 0.0
    assert run.get_range_stop_loss() == 0.0
    assert run.get_loss_trigger_price() == 0.0

    # 2. Production scenario: stepping down from 144.85 with step 30 and low below zero
    run._generate_grid(custom_high=144.85, custom_low=-50.0)

    # Ladder rungs must all be >= MIN_PREMIUM (e.g. 144.85, 114.85, 84.85, 54.85, 24.85; NO -5.15 rung)
    rungs = [s.level_price for s in run.grid_ladder]
    assert rungs == [144.85, 114.85, 84.85, 54.85, 24.85]
    assert all(lvl >= MIN_PREMIUM for lvl in rungs)
    assert -5.15 not in rungs

    # Stop loss must be clamped to >= 0.0 (and not -35.15)
    assert run.range_stop_loss >= 0.0
    assert run.get_range_stop_loss() >= 0.0
    assert run.get_loss_trigger_price() >= 0.0

    # Serialization must also clamp stop loss
    state = run.to_dict()
    assert state["range_stop_loss"] >= 0.0
    for slice_data in state["grid_ladder"]:
        assert slice_data["level_price"] >= MIN_PREMIUM


def test_regression_validate_config_rejects_negative_range_low():
    """
    Verifies that validate_config() rejects range_low < 0 or range_low < MIN_PREMIUM.
    """
    # Negative range_low
    if pytest:
        with pytest.raises(ValueError, match="range_low.*must be at least"):
            ScalperRunConfig(
                run_id="invalid_neg",
                range_high=100.0,
                range_low=-5.0,
    ...
    # range_low below MIN_PREMIUM
    if pytest:
        with pytest.raises(ValueError, match="range_low.*must be at least"):
            ScalperRunConfig(
                run_id="invalid_sub_min",
                range_high=100.0,
                range_low=0.01,
                slice_interval=10.0,
                profit_point=10.0,
    ...
    if pytest:
        with pytest.raises(ValueError, match="strictly less than range_high"):
            ScalperRunConfig(
                run_id="invalid_inverted",
                range_high=50.0,
                range_low=50.0,
                slice_interval=10.0,
                profit_point=10.0,
